import { SteamAccountClientStateCacheRepository, SteamAccountCredentials, User } from "core"
...
export function makeSACFactory(validSteamAccounts: SteamAccountCredentials[], publisher: Publisher) {
  return (user: User, accountName: string) => new SteamAccountClient({
    instances: {
...
export function makeUserClusterFactory(publisher: Publisher, sacStateCacheRepository: SteamAccountClientStateCacheRepository) {
  return (user: User) => new UserSACsFarmingCluster({
