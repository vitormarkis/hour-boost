...
describe("CreateSteamAccountController test suite", () => {
	test(
		"should add new account in case it exists on steam database",
		async () => {
			console.log = log
	...
	test(
		"should reject if provided account don't exist on steam database",
		async () => {
			console.log = log
			const dbMe = await usersRepository.getByID(ME_ID)
			expect(dbMe!.steamAccounts.data).toHaveLength(0)
	...
	test(
		"should reject when user attempts to add an account he already has",
		async () => {
			await sut.handle({
	...
	test(
		"should reject when user attempts to add an account that is already owned by other user",
		async () => {
			console.log = log
	...
	test(
		"should reject when user attempts to add more accounts than his plan allows",
		async () => {
			await sut.handle({
				payload: { accountName: "user1", password: "xx", userId: ME_ID },
			})
	...
	test(
		"should asks user for the steam guard",
		async () => {
			sut = new AddSteamAccountController(
				addSteamAccount,
				allUsersClientsStorage,
	...
	test.skip(
		"should ask for steam guard if asked",
		async () => {
