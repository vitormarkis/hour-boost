...
export type SteamAccountListItemProps = React.ComponentPropsWithoutRef<"div"> & {
	userId: string
	plan: PlanSession
	header?: boolean
	isFarming?: boolean
...
export const SteamAccountListItem = React.forwardRef<React.ElementRef<"div">, SteamAccountListItemProps>(
	function SteamAccountListItemComponent(
		{ accountPicture, steamGuard, isFarming, plan, userId, header, className, ...props },
		ref
	) {
		return (
			<div
			...
			>
				{header && (
					<div className="absolute left-4 bottom-full">
						<ModalAddSteamAccount userId={userId}>
				...
				<div className="relative flex items-center px-10 group cursor-default whitespace-nowrap">
					...
					<div className="flex flex-col">
						<div className="">
							<span className="leading-none text-[1rem]/[1rem]">2.4 </span>
              ...
              <span className="text-sm">horas</span> */}
				...
				<button className="relative flex items-center px-6 group hover:bg-slate-700 transition-all duration-300">
					...
					<div className="flex flex-col items-center">
						<span className="uppercase text-sm pb-1">1/{plan.maxGamesAllowed}</span>
...
SteamAccountListItem.displayName = "SteamAccountListItem"
...
export function SVGJoystick({ className, ...props }: SVGJoystickProps) {
	return (
		<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" {...props} className={cn("", className)}>
...
export function SGVDeviceMobile({ className, ...props }: SGVDeviceMobileProps) {
	...
	return (
		<svg {...props} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" className={cn("", className)}>
			<rect width={256} height={256} fill="none" />
			<rect
				x={64}
				y={24}
