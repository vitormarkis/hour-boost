...
>(function ModalAddSteamAccountComponent({ userId, children, className, ...props }, ref) {
	...
	return (
		<Dialog open={s.isModalOpen} onOpenChange={isOpen => (isOpen ? s.openModal() : s.closeModal())}>
			<DialogTrigger asChild>{children}</DialogTrigger>
			<DialogContent {...props} className={cn("border-slate-900", className)} ref={ref}>
				{/* <pre>
          {JSON.stringify(
            {
            ...
            2
				...
				<div className="pb-4">
					...
					>
						<div className="pb-1">
							<Label data-disabled={s.isRequiringSteamGuard} className="data-[disabled=true]:text-slate-500">
								Nome da conta
							</Label>
							<Input
				...
				<Button disabled={s.isSubmitting} onClick={handleSubmit} className="relative">
					{s.isSubmitting
	...
	return (
		...
		>
			<radialGradient id="a10" cx=".66" fx=".66" cy=".3125" fy=".3125" gradientTransform="scale(1.5)">
				<stop offset={0} stopColor={twc.slate["800"]} />
				<stop offset=".3" stopColor={twc.slate["800"]} stopOpacity=".9" />
				<stop offset=".6" stopColor={twc.slate["800"]} stopOpacity=".6" />
				<stop offset=".8" stopColor={twc.slate["800"]} stopOpacity=".3" />
				<stop offset={1} stopColor={twc.slate["800"]} stopOpacity={0} />
			</radialGradient>
			<circle
